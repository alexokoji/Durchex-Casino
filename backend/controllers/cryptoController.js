...
exports.tatumWebhook = async (req, res) => {
    try {
        ...
        if (!txData) {
            ...
            const lookupAddress = (currency.type === "erc-20" || currency.type === "bep-20") ? counterAddress : address;
            let walletData = await models.walletModel.findOne({ address: lookupAddress });
            console.log('tatumWebhook wallet lookup:', lookupAddress, walletData);
            if (walletData) {
                let userData = await models.userModel.findOne({ _id: walletData.userId });
                if (userData) {
                    userData.balance = userData.balance || { data: [] };
                    let balanceEntry = userData.balance.data.find((data) => data.coinType === currency.coinType && data.type === currency.type);
                    if (balanceEntry) {
                        balanceEntry.balance = Number(balanceEntry.balance || 0) + Number(amount);
                    } else {
                        userData.balance.data.push({ coinType: currency.coinType, balance: Number(amount), chain: currency.chain || '', type: currency.type || 'native' });
                    }
                    ...
                    console.warn('tatumWebhook: wallet has userId but user not found', walletData.userId);
                }
            } else {
                console.log('tatumWebhook: no wallet matched for incoming tx', txId, lookupAddress);
            }
        }
        // Always respond to the webhook to acknowledge receipt
